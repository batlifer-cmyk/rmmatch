...
function range(){const from=new Date();from.setHours(0,0,0,0);const to=new Date(from.getTime()+45*24*60*60*1000);return {from:dateKey(from),to:dateKey(to)};}
function jsonp(url,timeoutMs){
  return new Promise((resolve,reject)=>{
    ...
    const r=range();
    s.src=url+sep+'action=calendar.busy&callback='+encodeURIComponent(cb)+'&from='+encodeURIComponent(r.from)+'&to='+encodeURIComponent(r.to)+'&_='+Date.now();
    document.head.appendChild(s);
  });
}
function slotFromRow(row){
  if(!row||!row.start||!row.end)return null;
  const s=new Date(row.start),e=new Date(row.end);if(!Number.isFinite(s.getTime())||!Number.isFinite(e.getTime())||e<=s)return null;
...
async function fetchAll(force){
  ...
  state.promise=(async()=>{try{const data=await jsonp(endpoint(),18000);if(!data||data.ok!==true||data.schema!=='rm-calendar-live-v1'||!data.instructors)throw new Error(data&&data.message||'Apps Script 응답 형식 오류');state.data=data;state.fetchedAt=Date.now();state.error='';renderStatus();return data;}catch(e){state.error=e&&e.message||String(e);renderStatus();throw e;}finally{state.promise=null;}})();
  return state.promise;
}
async function slotsFor(id,force){const data=await fetchAll(!!force);if(!Object.prototype.hasOwnProperty.call(data.instructors,id))throw new Error('강사 일정 없음: '+id);return (Array.isArray(data.instructors[id])?data.instructors[id]:[]).map(slotFromRow).filter(Boolean);}
async function test(){await fetchAll(true);return true;}
function renderStatus(err){
  const el=document.getElementById('rm-apps-status');if(!el)return;const msg=err||state.error;
  ...
  el.textContent='RMHQ 배포 URL 내장됨 · 스케줄링 시 실시간 조회';el.style.color='var(--muted)';
}
function injectUi(){
  const page=document.getElementById('page-settings');if(!page||document.getElementById('rmAppsScriptCard'))return;
  const card=document.createElement('div');card.id='rmAppsScriptCard';card.className='card';
  card.innerHTML='<div class="card-title">Google Calendar · RMHQ Apps Script</div><div class="notice"><b>기본 실시간 소스</b>입니다. 스케줄링할 때 RMHQ 권한으로 5명 강사 원본 캘린더를 읽고 학생명 없이 BUSY 시간만 반환합니다.</div><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:10px"><button id="rm-apps-test" class="btn btn-primary btn-sm">실시간 연결 테스트</button><span id="rm-apps-status" style="font-size:12px"></span></div>';
  const anchor=document.getElementById('rmv2Safety')||page.querySelector('.sec-sub');if(anchor)anchor.insertAdjacentElement('afterend',card);else page.prepend(card);
  card.querySelector('#rm-apps-test').onclick=async()=>{try{await test();renderStatus();if(typeof testAllICS==='function')await testAllICS();}catch(e){renderStatus(e.message||String(e));}};renderStatus();
}
