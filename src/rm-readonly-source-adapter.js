...
function adaptMasterTimeDataRow(row, sourceRow) {
  ...
  return {
    sourceType: 'lesson', sourceSheet: 'Master time data', sourceRow,
    instructorName: text(teacher), studentName: text(student), lessonDate: text(date),
    month: text(month), day: text(day), quarter: text(quarter), hours: numericHours,
    code, rate: numberValue(rate), totalAmount: numberValue(totalAmount),
    classType: text(classType), note: text(note),
...
function adaptRegistrationLogRow(row, sourceRow) {
  ...
  const transactionKind = classifyRegistrationTransaction(rawMessage, parsedCount);
  const suspiciousCount = parsedCount !== null && (parsedCount <= 0 || parsedCount > 100) && transactionKind === 'UNKNOWN_999';
  const sourceStatus = normalizedStatus(status);

  return {
    sourceType: 'registration', sourceSheet: '_DB_등록로그', sourceRow,
    registeredAt: text(registeredAt), studentName: text(studentName), phone: text(phone),
    amount: numberValue(amount), packageCount: parsedCount, instructorName: text(instructor),
    enteredBy: text(enteredBy), rawMessage: text(rawMessage), status: sourceStatus,
    lessonType: text(lessonType), transactionKind,
    excludedFromPackageTotals: ['CANCELLED', 'NON_PACKAGE_ADJUSTMENT'].includes(transactionKind),
...
function adaptGraduationLogRow(row, sourceRow) {
  ...
  return {
    sourceType: 'graduation', sourceSheet: '_DB_졸업로그', sourceRow,
    processedAt: text(processedAt), studentName: text(studentName), lastLessonDate: text(lastLessonDate),
    remainingLessons: numberValue(remaining), reason: text(reason), processedBy: text(processedBy),
    rawMessage: text(rawMessage), status: normalizedStatus(status),
...
function adaptPaymentLogRow(row, sourceRow) {
  ...
  return {
    sourceType: 'payment', sourceSheet: '입금로그', sourceRow,
    receivedAt: text(receivedAt), paidAt: text(paidAt), payerName: text(payerName),
    amount: numberValue(amount), addedCount: parsedCount, instructorType: text(instructorType),
    judgment: text(judgment), targetRow: text(targetRow), processStatus: text(processStatus),
    jandiStatus: text(jandiStatus), rawMessage: text(rawMessage), memo: text(memo),
...
module.exports = {
  numberValue, isTemporaryStudentLabel, rowHasData, classifyRegistrationTransaction,
  adaptMasterTimeDataRow, adaptRegistrationLogRow, adaptGraduationLogRow,
  adaptPaymentLogRow, adaptRows,
};
