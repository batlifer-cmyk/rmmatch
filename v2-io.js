...
function rmv2InjectSettingsNotice(){
  ...
  box.innerHTML='<div class="card-title">RM Scheduler v2 운영 안전패치</div>'+ '<div class="notice">실제 날짜·수업 길이·반복 일정·임시 일정·최대 2인 강사를 구분합니다. 임시 일정 1회 때문에 매주 같은 시간이 영구 차단되지 않습니다.</div>'+ '<div style="font-size:12px;color:var(--muted);line-height:1.7;margin-bottom:12px">강사 설정은 아직 이 브라우저 localStorage에 저장됩니다. 다른 PC로 옮길 때 아래 백업/가져오기를 사용하세요. Anthropic API Key는 v2부터 localStorage에 남기지 않고 현재 탭 세션에만 보관합니다.</div>'+ '<div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn btn-ghost btn-sm" id="rmv2Export">강사 설정 백업</button><label class="btn btn-ghost btn-sm" style="cursor:pointer">강사 설정 가져오기<input id="rmv2Import" type="file" accept="application/json" style="display:none"></label></div>';
